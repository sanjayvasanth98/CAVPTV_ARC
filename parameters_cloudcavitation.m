function parameters = parameters_CAVPTV(videoFile,optimizedParams)
    ...
    parameters.analysis_mode = 1;  % 1 for single file, 2 for batch processing
    parameters.analysis_type = 2;  % 1 for Particle Analysis, 2 for Blob Analysis
   ...
   % Define Analysis Type
    parameters.analysisType = 2; % '1' for Particle Analysis, '2' for Blob Analysis 
   % Define sampleType
    parameters.sampleType = 3;  % '1' for X-ray Samples, '2' for Backlight Samples with particles, '3' for Backlight Samples Without Particles
    ...
    parameters.savevideopreprocess = true;
    parameters.xrayillumination = false; % logical input for indicating that the type of illumination for process variations
    parameters.flatfieldcorr = true;
    parameters.sigma = 80; %sigma value for flat-field correction
    ...
    parameters.dt = 9.81e-6; % Pixels per meter (Backlight)
    parameters.px = 264180; % Pixels per meter (Backlight)
    parameters.computeFlow = false;   % or false to skip estimateFlow + flow .mat
    parameters.xrayill = 5; % 1=Xray images 2=backlight images with particles 3=backlight images without particles 4= segementing all size bubbles from backlight videos, 5 = cloud cavitation
